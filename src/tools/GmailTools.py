...
class GmailToolsClass:
    ...
    def create_draft_reply(self, initial_email, reply_text):
        try:
            print(f"\nCreating draft reply to: {initial_email.sender}")
            ...
            print(f"Initial email sender: {initial_email.sender}")
            print(f"Initial email subject: {initial_email.subject}")
            return None

    def send_reply(self, initial_email, reply_text):
        try:
            print(f"\nSending reply to: {initial_email.sender}")
            print(f"Subject: {initial_email.subject}")
    ...
    def _create_html_email_message(self, recipient, subject, reply_text):
        ...
        # 从收件人字符串中提取电子邮件地址
        # 处理可能的格式：Name <email@example.com> 或 email@example.com
        email_match = re.search(r'[\w\.-]+@[\w\.-]+', recipient)
        if not email_match:
            print(f"Warning: Invalid recipient format: {recipient}")
            # 如果无法提取有效的电子邮件地址，使用原始收件人
            message["to"] = recipient
        else:
            message["to"] = email_match.group(0)
            
        message["subject"] = f"Re: {subject}" if not subject.startswith("Re: ") else subject
        message["from"] = os.environ.get('MY_EMAIL', 'me@gmail.com')  # 使用环境变量或默认值
